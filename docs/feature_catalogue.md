...
Static and lifetime-aggregate features. Refreshed daily (not via DT).
...
Rolling-window aggregates per customer. Computed in `CUSTOMER_VELOCITY` Dynamic Table.
...
Rolling aggregates at the merchant level (across all customers). Computed in `MERCHANT_VELOCITY` Dynamic Table.
...
Rolling aggregates at the DPAN level. Computed in `WALLET_DPAN_VELOCITY` Dynamic Table.
...
Rolling aggregates at the IP level. Computed in `IP_VELOCITY` Dynamic Table.
...
Computed downstream from velocity DTs. These capture ratios and behavioural patterns.
